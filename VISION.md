...
> Principles marked with **[live]** are implemented. Those marked with **[planned]** describe where the mod is headed; see DEVELOPMENT.md "Known Limitations & Open Questions" for the technical roadmap.
...
> *Material donation API and structure registration API are both live via `VillageBuilderAPI`. Third-party structures enter the same candidate pool as vanilla structures.*
...
**Mod-registered structures:** Other mods register structures with metadata: what need they satisfy, what materials they require, and optionally what biomes they prefer. These enter the candidate pool alongside vanilla structures and are weighed the same way.
...
**Not a progression system.** There's no tech tree, no unlocks, no "you must build X before Y" beyond natural need priorities. *(Builder trade tiers do gate higher-cost plans and materials behind villager level-ups, providing a soft sense of progression; the village itself still grows without gating.)*
