...
public class BuilderTrades {
    ...
    // ---------------------------------------------------------------------------
    // buildTradeTable: level→TradeSet ResourceKey map for VillagerProfession
    //
    // Villager trades are fully data-driven: the profession holds
    ...
    public static List<MerchantOffer> buildDynamicOffers(ServerLevel world, BlockPos villagerPos, int villagerLevel) {
        ...
        // Level 5: flag stake, which lets the player designate the next build site
        if (villagerLevel >= 5) {
     ...
     * - needed and <25% stocked → rate halved (player gives fewer items, so the trade is more valuable)
